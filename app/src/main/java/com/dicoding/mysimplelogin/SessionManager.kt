...
class SessionManager(context: Context) {
    ...
    private var pref: SharedPreferences = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
        ...
        val masterKey = MasterKey.Builder(context) // membuat Master Key sendiri
            .setKeyGenParameterSpec(spec)
            .build()
        EncryptedSharedPreferences.create( // untuk membuat Enkripsi
            context, "Session", masterKey,
            EncryptedSharedPreferences.PrefKeyEncryptionScheme.AES256_SIV, // MasterKey sendiri dengan spesifikasi algoritma AES GCM 256,
            EncryptedSharedPreferences.PrefValueEncryptionScheme.AES256_GCM
        )
    } else SecurePreferences(context) // enkripsi dengan SecurityPreference
